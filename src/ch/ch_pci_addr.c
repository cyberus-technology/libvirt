 ...
 *
 * ch_pci_addr.c: Manage Cloud-Hypervisor PCI device addresses
 *
 ...
 */
static int chReserveOrQueueForPciSlotId(virDomainPCIAddressSet *addrSet, virDomainDeviceInfo *devInfo, dynamicAddressQueue* queue) {
    ...
    if (1 == virDeviceInfoPCIAddressIsWanted(devInfo)) {
        // ... add it to the queue for later slot id assignment to prevent conflicts
        if (0 != add_info_to_queue(queue, devInfo)) {
            virReportError(VIR_ERR_INTERNAL_ERROR,
                _("Failed to add device '%s' to the queue for PCI ID assignment!"), devInfo->alias);
            return -1;
        }
        DBG("Queued device '%s' for later PCI slot assignment", devInfo->alias);
    // ... else, if the device is not configured to be of any type...
    } else {
        if ((devInfo->type != VIR_DOMAIN_DEVICE_ADDRESS_TYPE_NONE) && (devInfo->type != VIR_DOMAIN_DEVICE_ADDRESS_TYPE_PCI)) {
            virReportError(VIR_ERR_INTERNAL_ERROR,
                _("Found non-PCI device '%s'. CHV only supports PCI virtio devices!"), devInfo->alias);
            return -1;
        }
        // We don't need to assign a new slot, but must mark the slot as taken
        if (0 != virDomainPCIAddressReserveAddr(addrSet, &devInfo->addr.pci, devInfo->pciConnectFlags, 0)) {
            virReportError(VIR_ERR_INTERNAL_ERROR,
                _("Couldn't reserve PCI slot %d from config of device with alias '%s'!"), devInfo->addr.pci.slot, devInfo->alias);
            return -1;    
        }
 ...
 * Returns: 0 on success, -1 in case of error 
 */
static int chInitNetworkVirtioPciDevices(virDomainPCIAddressSet *addrSet, size_t numDefs, virDomainNetDef **netDevDefs, dynamicAddressQueue* queue) {
    size_t idx;
    for (idx = 0; idx < numDefs; ++idx) {
        virDomainNetDef *net = netDevDefs[idx];
        if (net->type != VIR_DOMAIN_NET_TYPE_ETHERNET) {
            virReportError(VIR_ERR_INVALID_NETWORK,
                _("CHV driver only supports `ethernet` network types!"));
            return -1;
        }
        if (!virDomainNetIsVirtioModel(net)) {
            VIR_WARN("Found non PCI net device with model type %s", virDomainNetModelTypeToString(net->model));
            continue;
        }
 ...
 * Returns: 0 on success, -1 in case of error 
 */
static int chInitDiskVirtioPciDevices(virDomainPCIAddressSet *addrSet, size_t numDefs, virDomainDiskDef **diskDevDefs, dynamicAddressQueue* queue) {
    size_t idx;
    for (idx = 0; idx < numDefs; ++idx) {
...
/**
 * Initialize all serial devices in the domain with an PCI slot ID or places them in a queue for later assignment.
 *
 * Device without a slot ID given in the XML are queued for later ID assignment and therefore not initialized by this 
 * function.
 *
 * @addrSet: PciAddressSet to reserve the slot IDs from
 * @numDefs: Length of the serial device array
 * @deviceDevs: Array of serial devices definitions of a domain
 * @queue: Queue of DeviceInfos for which a slot ID is reserved later
 *
 * Returns: 0 on success, -1 in case of error 
 */
static int chInitSerialVirtioPciDevices(virDomainPCIAddressSet *addrSet, size_t numDefs, virDomainChrDef **charDevDefs, dynamicAddressQueue* queue) {
    size_t idx;
    for (idx = 0; idx < numDefs; ++idx) {
        virDomainChrDef *device = charDevDefs[idx];
        // Device is neither `None` nor VIR_DOMAIN_CHR_SERIAL_TARGET_TYPE_PCI, so incompatible
        if (device->targetType & (~VIR_DOMAIN_CHR_SERIAL_TARGET_TYPE_PCI)) {
            VIR_WARN("Found unsupported non-PCI target type for serial: %s", virDomainChrSerialTargetTypeToString(device->targetType));
            continue;
        }
        // Check if device has a compatible mode; expect either 0 or PCI_SERIAL
        if (device->targetModel & (~VIR_DOMAIN_CHR_SERIAL_TARGET_MODEL_PCI_SERIAL)) {
            VIR_WARN("Found unsupported non-PCI target model of: %d", device->targetModel);
            continue;
        }
        // Set the expected model and type
        device->targetType = VIR_DOMAIN_CHR_SERIAL_TARGET_TYPE_PCI;
        device->targetModel = VIR_DOMAIN_CHR_SERIAL_TARGET_MODEL_PCI_SERIAL;
        if (chReserveOrQueueForPciSlotId(addrSet, &device->info, queue))
            return -1;
    }
    return 0;
}
...
static int chReserveDynamicPciIds(virDomainPCIAddressSet *addrSet, dynamicAddressQueue *queue) {
    for (size_t idx = 0; idx < queue->nextIdx; ++idx) {
        ...
        DBG("Assigned new Slot %d to device '%s' without BDF", queue->devInfos[idx]->addr.pci.slot, queue->devInfos[idx]->alias);
    }
    return 0;
...
int chInitPciDevices(virDomainObj *dom) {
    ...
    */ 
    ...
    if (chInitSerialVirtioPciDevices(addrSet,def->nserials, def->serials, queue)) {
        rc = -1;
        VIR_ERROR("%s:%d: Failed to attach Virtio serial devices", __FILE_NAME__, __LINE__);
        goto cleanup;
    }
 ...
 * Succeeds if after successfully assigning an address to the device.
 * 
 * 0 indicates success, -1 failure 
 */
...
void chDomainReleaseDeviceAddress(virDomainObj *vm, virDomainDeviceInfo *info)
{
    ...
    if (1 == virDeviceInfoPCIAddressIsPresent(info)) {
